...
from fantasy_agent.automation.automation_store import AutomationStore, InvalidContract, instant, utc
from fantasy_agent.weekly.weekly_data import context, state_key, fingerprint, load, collect, acquisition_summary
from fantasy_agent.weekly.lineup_validation import check_document, validate
from fantasy_agent.core.storage import save_new
from fantasy_agent.weekly.weekly_model import ELIGIBLE
...
class Execution:
    ...
    def read(self, path):
        ...
        value = load_config(self.root) if proof['path'] == 'config.json' else json.loads((self.root / proof['path']).read_text())
        return value, proof
    ...
    def active(self, db, execution_id):
        ...
        if proposal['league_id'] != load_config(self.root)['league_id']:
            raise InvalidContract('Execution belongs to another configured league')
        if value.get('expected_user_id') != load_config(self.root)['user_id']:
            raise InvalidContract('Execution belongs to another configured user')
        ...
        if authority['mode'] == 'delegated':
            from fantasy_agent.execution.lineup_authority import verify
            proposal, _ = self.read(value['proposal']['path'])
            verify(self, db, proposal, authority)
        if authority['mode'] == 'manager':
            from fantasy_agent.execution.manager_authority import verify
            verify(self, db, proposal, authority, 'unlocked_starters')
        return row, value
    ...
    def register(self, proposal_path, authority_path):
        ...
        if authority.get('schema_version') != 1 or authority.get('mode') not in ('supervised', 'delegated', 'manager') or authority.get('proposal_hash') != phash:
    ...
    def check_pair(self, api, native, proposal, *, after=None):
        ...
        if (api.get('user', {}).get('user_id') != config['user_id']
                or api['roster'].get('owner_id') != config['user_id']
                or native.get('account_menu_observed') is not True
                or native.get('account') != api.get('user', {}).get('username')):
            raise InvalidContract('Observe the signed-in account menu and match the configured Sleeper user ID')
    ...
    def prepare(self, execution_id, snapshot, api_path, native_path, review_path=None):
        ...
        with self.db() as db:
            ...
            if authority['mode'] in ('delegated', 'manager') and (result['status'] != 'PASS' or review_path):
    ...
    def dispatch(self, action_id, token):
        with self.db() as db:
            ...
            if authority['mode'] == 'manager':
                from fantasy_agent.execution.manager_authority import verify
                proposal, _ = self.read(execution['proposal']['path'])
                verify(self, db, proposal, authority, 'unlocked_starters', dispatch=True)
            if authority['mode'] == 'delegated':
                from fantasy_agent.execution.lineup_authority import verify
                proposal, _ = self.read(execution['proposal']['path'])
    ...
    def reconcile(self, action_id, api_path, native_path):
        ...
        from fantasy_agent.notifications.team_notifications import after_change
        notice = after_change(self.root)
        return {'action_id': action_id, 'outcome': outcome, 'execution_state': state, 'blind_retry_allowed': False, 'notification': notice}
    ...
    def recover(self, action_id, evidence_path):
        ...
        with self.db() as db:
            ...
            automatic = evidence.get('basis') == 'automatic_unused_token_recovery' and row['state'] == 'armed'
            if evidence.get('action_id') != action_id or (evidence.get('basis') != 'explicit_owner_recovery' and not automatic) or not evidence.get('reason'):
    ...
    def status(self):
        with self.db() as db:
            from fantasy_agent.execution.lineup_authority import policy
            current = policy(db)
            enabled = current['enabled']
            if enabled:
                try:
                    from fantasy_agent.execution.lineup_authority import checked
    ...
    def reauthorize(self, execution_id, authority_path):
        ...
        with self.db() as db:
            ...
            if authority.get('mode') == 'manager':
                from fantasy_agent.execution.manager_authority import verify
                verify(self, db, proposal, authority, 'unlocked_starters')
            elif authority.get('mode') != 'supervised' or authority.get('proposal_hash') != value['proposal_hash'] or owner.get('basis') != 'explicit_owner_approval' or owner.get('proposal_hash') != value['proposal_hash']:
                raise InvalidContract('Renewal requires exact owner approval or standing manager authority')
...
def main(argv=None):
    ...
    try:
        ...
        if args.command == 'observe':
            ...
            from fantasy_agent.execution.lineup_authority import promote
            result = promote(execution, args.delegation, args.tests)
        elif args.command == 'revoke':
            from fantasy_agent.execution.lineup_authority import revoke
            result = revoke(execution, args.reason)
        elif args.command == 'authorize-delegated':
            from fantasy_agent.execution.lineup_authority import authorize
