...
def checked(engine, db, proposal):
    ...
    if config['league_id'] != proposal['league_id']:
    ...
    if current.get('expected_user_id') != config['user_id']:
        raise InvalidContract('Standing lineup authority belongs to another configured user')
