...
from fantasy_agent.weekly.weekly_data import context, fingerprint
...
class RosterOperations(Execution):
    ...
    def register_plan(self, plan_path, approval_path):
        ...
        if (plan.get('schema_version') != 1 or plan.get('mode') != 'supervised'
                or approval.get('basis') not in ('explicit_owner_transaction_approval', 'standing_manager_authority')
        ...
        with self.db() as db:
            tables(db)
            if approval.get('basis') == 'standing_manager_authority':
                from fantasy_agent.execution.manager_authority import verify
                for step in plan['steps']:
                    verify(self, db, plan, approval, step['kind'])
    ...
    def plan(self, db, plan_id):
        ...
        if plan['league_id'] != load_config(self.root)['league_id']:
            raise InvalidContract('Transaction plan is for another configured league')
        if value.get('expected_user_id') != load_config(self.root)['user_id']:
            raise InvalidContract('Transaction plan is for another configured user')
        return value, plan
    ...
    def prepare_step(self, plan_id, ordinal, api_path, native_path, valuation_path=None):
        with self.db() as db:
            ...
            managed = approval.get('basis') == 'standing_manager_authority'
            if managed:
                from fantasy_agent.execution.manager_authority import verify
                verify(self, db, plan, approval, step['kind'])
            ...
            if kind in ('waiver', 'free_agent'):
                ...
                if managed and (computed['net_policy_value'] is None or computed['net_policy_value'] <= 0):
                    raise InvalidContract('Automatic acquisition requires positive value after priority cost')
    ...
    def dispatch_step(self, action_id, token):
        with self.db() as db:
            ...
            if approval.get('basis') == 'standing_manager_authority':
                from fantasy_agent.execution.manager_authority import verify
                verify(self, db, plan, approval, action['step']['kind'], dispatch=True)
    ...
    def reconcile_step(self, action_id, api_path, native_path):
        ...
        return {'action_id': action_id, 'outcome': outcome, 'blind_retry_allowed': False, 'notification': notice}
    ...
    def retire_unused(self, action_id, evidence_path):
        ...
        with self.db() as db:
            ...
            if evidence.get('basis') not in ('explicit_owner_recovery', 'automatic_unused_token_recovery') or evidence.get('action_id') != action_id or not evidence.get('reason'):
