...
from fantasy_agent.automation.automation_store import AutomationStore, AutomationError
...
def main(argv=None):
    ...
    try:
        if args.command.startswith('schedule-'):
            ...
            from fantasy_agent.automation.automation_run import browser_observation
            result = browser_observation(store.root, args.status, args.evidence)
        elif args.command == 'incorporate-rules':
            from fantasy_agent.automation.automation_deadlines import incorporate_saved
            ...
            config = load_config(path=args.config or store.root / 'config.json')
