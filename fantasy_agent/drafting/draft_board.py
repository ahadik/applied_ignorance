...
from fantasy_agent.drafting.draft_inputs import ROOT, load_inputs
from fantasy_agent.drafting.draft_history_analysis import build as build_history, timestamp
from fantasy_agent.core.storage import save_atomic

from fantasy_agent.core.player_scoring import (POSITIONS, OFFENSE, STAT_MAP, OFFENSE_EXTRA, DEFENSE_STATS,
...
def export_candidates(board, current):
    ...
    if board.get('draft_session'):
        from fantasy_agent.drafting.draft_session import verify_board
...
def main():
    ...
    try:
        if args.command == 'build':
            from fantasy_agent.providers.provider_freeze import LOCK
