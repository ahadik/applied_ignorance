...
from fantasy_agent.drafting.draft_data import read_draft_context
from fantasy_agent.providers.fantasypros import FantasyPros, get_fantasypros
...
def collect(season, refresh=False):
    from fantasy_agent.providers.provider_freeze import check
...
def publish_context(context, config):
    from fantasy_agent.drafting.draft import summary
...
def main():
    ...
    try:
        if args.command == 'adopt-context':
            ...
            publish_context(context,load_config(ROOT))
            ...
            publish_context(inputs['context'], load_config(ROOT))
