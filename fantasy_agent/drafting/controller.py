...
import time
from fantasy_agent.drafting.draft import snake_picks
from fantasy_agent.drafting.draft_data import read_live_draft
...
def rank(state, candidates):
    ...
    if 'state_fingerprint' in candidates:
        from fantasy_agent.drafting.draft_strategy import fingerprint
...
def update_strategy(folder,state,board_path,policy_path=None,*,allow_mock=False):
    ...
    import hashlib
    from fantasy_agent.drafting.draft_strategy import ENGINE_VERSION, Engine, candidate_export, fingerprint, load_policy
...
def main():
    ...
    folder = PROJECT_ROOT / 'data' / 'drafts' / args.draft
