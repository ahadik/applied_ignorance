...
from fantasy_agent.weekly.weekly_data import load, fingerprint
from fantasy_agent.weekly.weekly_model import build, optimize, ELIGIBLE, schedule_games
...
def pool_from_snapshots(paths, now):
    return pool_from_inputs([load(Path(path)) for path in paths], paths, now)


def pool_from_inputs(inputs, paths, now):
    """Reuse already checksum-verified inputs within one review, retaining source age checks."""
    if not inputs or len(inputs) != len(paths):
        raise InvalidContract('Match each verified input with its snapshot path')
    reports = [build(value, now, analysis_pool=True) for value in inputs]
    ...
    first = reports[0]
    from fantasy_agent.execution.roster_operations import rules_key
    ctx = inputs[0]['final_context']
