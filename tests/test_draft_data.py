...
from fantasy_agent.drafting.draft import sync
from fantasy_agent.drafting.draft_data import read_draft_context, read_live_draft
from fantasy_agent.core.storage import save_atomic


class DraftDataTests(unittest.TestCase):
    def setUp(self):
        self.config = {'league_id': '123', 'username': 'user', 'user_id': 'u'}
    ...
    def test_context_composes_shared_reads_and_preserves_snapshot_shape(self):
        with patch('fantasy_agent.drafting.draft_data.get_sleeper', side_effect=[self.league, self.draft,
                                                        {'user_id': 'u'}, [], []]) as get:
    ...
    def test_wrong_draft_fails_before_downstream_reads(self):
        with patch('fantasy_agent.drafting.draft_data.get_sleeper', side_effect=[self.league, {'draft_id': '999'}]) as get:
    ...
    def test_failed_sync_preserves_existing_snapshot(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch('fantasy_agent.drafting.draft.SNAPSHOT', destination), patch('fantasy_agent.drafting.draft_data.get_sleeper',
    ...
    def test_live_board_requires_known_untraded_ownership(self):
        for trades in (None, {}, [{'roster_id': 2}]):
            with patch('fantasy_agent.drafting.draft_data.get_sleeper', side_effect=[self.draft, trades]) as get:
                with self.assertRaises(ValueError):
                    read_live_draft('456')
            self.assertEqual(get.call_count, 2)

    def test_live_board_disables_retries_on_all_reads(self):
        with patch('fantasy_agent.drafting.draft_data.get_sleeper', side_effect=[self.draft, [], []]) as get:
