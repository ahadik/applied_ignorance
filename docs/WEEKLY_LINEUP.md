...
September 9 checkpoint: all 182 project tests passed. Final live run at 04:44 UTC
used 11 Sleeper network attempts, nine FantasyPros cache hits with zero new FP
requests, and one nflverse metadata request with cached schedule bytes. The
04:45 UTC read-back verification correctly reported `matches=false`: Mahomes
remained the actual QB, Purdy the recommended QB, and the observed league/roster
state had not changed since analysis. No recommendation was submitted.

Run `python3 -m unittest discover -v`. Tests use simulated providers and temporary
storage. They cover original draft-scoring compatibility, brute-force optimizer
agreement, multi-position/FLEX legality, fixed/off-roster starters, started bench
exclusion, injuries, byes, stale/wrong-week data, identities, schedule completeness,
time zones, postponed kickoff conservatism, provenance/revision reuse and tampering.
