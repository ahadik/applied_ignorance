...
Read before drafting and after every compaction, reconnect, or interruption. Based on the two completed mocks linked below. The read-only monitor and selection-preparation checks are now implemented in `controller.py`; follow [controller instructions](CONTROLLER.md). Browser queue maintenance remains supervised, and these safeguards are not a guarantee against failure.
...
1. Reconcile new picks and current roster from fresh state; recompute candidates with deterministic tools where implemented. Strategic reasoning should use those outputs.
...
Not verified: offline queue persistence, actual-league timeout behavior, queue-panel draft button, drag ordering, automated queue maintenance, and a fully integrated deterministic recommendation loop.
