...
`Engine.opponent_options` pre-indexes rankings by position. Since scenario and need
adjustments are constant within a position, only each position's leading eligible
options are needed to reproduce the original global top-five ranking exactly.
