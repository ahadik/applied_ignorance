...
2. **Cloud monitoring and phone access.** Package the shared services, add durable jobs and the central model client, deploy private storage after data-scope approval, then implement MCP and notifications. Exit gate: with the Mac off and chat closed, cloud work continues; the phone retrieves the same durable job through native MCP or the documented fallback. Mac execution is correctly shown as unavailable.
