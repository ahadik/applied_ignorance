...
from lineup_history import append_proposal
from lineup_validation import proposal_from_report, validate
from roster_operations import collect as collect_rosters, RosterOperations
from season_strategy import pool_from_snapshots, compare, season_calendar
from storage import save_new, save_atomic
from weekly_data import collect, load, acquisition_summary
from weekly_model import build
...
def replay(root, result_path):
    ...
    pool = pool_from_snapshots([source['snapshot']], as_of)
...
def review(root, season, week, *, notify=False, checkpoint=lambda: None):
    ...
    with (root / 'data/agent_cycles/review.lock').open('a') as lock, (weekly / 'operation.lock').open('a') as weekly_lock:
        ...
        try:
            ...
            pool = pool_from_snapshots([snapshot], now)
        ...
        if notify:
            from pushover import get_pushover
            try:
